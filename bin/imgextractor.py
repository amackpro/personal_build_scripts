...
class Extractor(object):
                ...
                if entry_inode.is_dir:
                    ...
                    if os.name == 'posix':
                        ...
                        try:
                            ...
                            # Skip ownership change if not permitted (e.g., for lost+found)
                            pass
                    ...
                    if os.name == 'posix':
                        ...
                        try:
                            ...
                            # Skip ownership change if not permitted
                            pass
                    if cap == '' and con == '':
                        tmppath=self.FileName + entry_inode_path
                        if (tmppath).find(' ',1,len(tmppath))>0:
    ...
    def __converSimgToImg(self, target):
        with open(target, "rb") as img_file:
            ...
            # Create temporary raw image file
                    ...
                    if chunk_header.type == 0xCAC1:  # CHUNK_TYPE_RAW
                        ...
                        if chunk_header.type == 0xCAC2:  # CHUNK_TYPE_FILL
                            ...
                            if chunk_header.type == 0xCAC3:  # CHUNK_TYPE_DONT_CARE
                                ...
                                sector_base += sector_size
                    total_chunks -= 1
        # Remove the original sparse image
        os.remove(target)
        # Rename raw image to original filename
        os.rename(raw_img_path, target)
        # Update the OUTPUT_IMAGE_FILE to point to the renamed file
